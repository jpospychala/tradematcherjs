...
Storage.prototype.writeOffer = function(offer) {
  return this.query('INSERT INTO offers (oid,product,price,amount,operation) VALUES($1, $2, $3, $4, $5)',
  [offer.oid, offer.product, offer.price, offer.amount, offer.is]);
}
...
Storage.prototype.load = function() {
  ...
  .then(function(result) {
    ...
    result.rows.forEach(function(item) {
      var a = item.product;
      var b = item.price;
      var c = item.operation;
      model[a] = model[a] || {};
      model[a][b] = model[a][b] || {};
      model[a][b][c] = model[a][b][c] || [];
      model[a][b][c].push({oid: item.oid, amount: parseFloat(item.amount)});
    });
    return model;
  });
}
...
Storage.prototype.migrate = function() {
  this.query(
    "CREATE TABLE IF NOT EXISTS offers (oid int PRIMARY KEY, product VARCHAR(128), price NUMERIC(6), amount NUMERIC(6), operation VARCHAR(8))");
}
